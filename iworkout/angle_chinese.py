        ...
        if len(lmList) != 0:
            ...
            if abs(int(right_sum - left_sum)) <= 30:
                if abs(int(deadlift_mins['deadlift_hip_to_ankle_right'] - deadlift_mins['deadlift_hip_to_ankle_left'])) <= 10:
                    if abs(int(deadlift_mins['deadlift_head_to_ankle_right'] - deadlift_mins['deadlift_head_to_ankle_left'])) <= 20:
                        if abs(int(deadlift_mins['deadlift_head_to_hip_right'] - deadlift_mins['deadlift_head_to_hip_left'])) <= 20:
                            score += 100
...
if __name__ == "__main__":
    print(return_angle_squat(cv2.imread('raw_data/test_img/squat/bad_squat2.jpg')))
